...
int main(int argc, char** argv)
{
	...
	std::cout << "[oidc_vault_minio_test] Start test for liboidc-vault-minio.so" << std::endl;
	std::cout << std::endl;
	...
	if(!InitS3fsCredential("Debug", &perrstr)){
		std::cerr << "     [ERROR] Could not initialize liboidc-vault-minio.so : " << (perrstr ? perrstr : "unknown") << std::endl;
		if(perrstr){
			free(perrstr);
		}
		exit(EXIT_FAILURE);
	...
	if(!UpdateS3fsCredential(&paccess_key_id, &pserect_access_key, &paccess_token, &token_expire, &perrstr)){
		std::cerr << "     [ERROR] Could not get Credential for MinIO : " << (perrstr ? perrstr : "unknown") << std::endl;
		if(perrstr){
			free(perrstr);
		}
		FreeS3fsCredential(&perrstr);
	...
	std::cout << "                 S3 Access Key Id     = " << paccess_key_id		<< std::endl;
	std::cout << "                 S3 Secret Key        = " << pserect_access_key		<< std::endl;
	std::cout << "                 OIDC Access Token    = " << paccess_token		<< std::endl;
	...
	if(!FreeS3fsCredential(&perrstr)){
		std::cerr << "     [ERROR] Could not uninitialize liboidc-vault-minio.so : " << (perrstr ? perrstr : "unknown") << std::endl;
		if(perrstr){
			free(perrstr);
		}
		exit(EXIT_FAILURE);
